...
`test/procgen/invariants.ts`, measured off the **drawn** Catmull-Rom curve,
not off the control points.

Epsilon is game-derived: `2 * PLAYER_RADIUS` (1.24 m) — a child's width, the
same derivation `WALKABLE_GAP` already uses. Not the generator's own numbers.

Structural note for whoever follows: the past-the-doormat extension can never
exceed **0.4 m**. `pastReach = min(2, l - edge - PAST_CLEARANCE)` and
`l = edge + standOff`, so it collapses to `min(2, 1.4 - 1) = 0.4` for
everything with a footprint. That is why 1.24 m is comfortable and still tight
enough to have caught a 3.4 m miss.
...
- [ ] `npm run build` + `npm run test:procgen` green

Throwaway audit script lives at `scripts/zz-audit-paths.mts` — **delete before
raising the PR**, it is scaffolding, not a check.
