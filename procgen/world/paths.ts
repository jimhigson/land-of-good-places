import { PARK_LAYOUT, RING_RADIUS } from '../../src/world/parkLayout';
import { MAIN_LOOP_WIDTH, PATH_KERB_OVERHANG, PLAYER_RADIUS } from '../../src/core/constants';
import { lazyArrayView } from '../../src/boot/lazyView';
import { RAIL_RACE_PLAN } from '../../src/world/railRace/plan';
import { archFeet } from '../../src/world/railRace/arch';
import { CROSSING_SITES, type CrossingSite } from '../../src/world/train/crossingPlan';
import { DECK_HALF_LENGTH } from '../../src/world/train/bridgeFootprint';
import { RAIL_CORRIDOR_CLEARANCE as RAIL_CORRIDOR_CLEARANCE_PLAN, TRAIN_PLAN } from '../../src/world/train/plan';
import { Vector3 } from 'three';
import { PARK_BOUNDARY } from '../../src/world/boundary';
import { SLIDE_PLAN } from '../../src/world/slide/plan';
import { ARRIVAL_EXEMPT_NEAR, DEPARTURE_EXEMPT_NEAR } from '../../src/world/streetRules';
import { ENTRANCE_GATE_Z } from '../../src/world/entrance/layout';
...
import { JUNCTION_SNAP, PLAZA, curvePoints, latticeStateSnapshot, pathDivisions, pavedLatticeEdges, pavedLatticeNodes, restoreLatticeState, routeCurve, tapRimsDrawn, type Blocker, type LatticeNeighbour, type LatticeStateSnapshot, type LatticeTap, type PathEdge, type PathGraph, type PathNode, type RouteDefinition, type StreetLattice, type StreetStub, usedTaps } from '../../src/world/paths';
/**
 * **The path network's search** — the ring, the street lattice, every spur's
 * route and the interconnects. Moved verbatim from `src/world/paths.ts`,
 * which keeps the path types, the paving state the drawing reads and the
 * drawing itself. Build-time only (`docs/design/PREBUILT-PARKS.md`).
 */
...
/**
 * Forget every memo the path search keeps — what `resetPathsState` used to
 * clear alongside the paving before the search moved here. Registered as a
 * plan cache, so an unwind forgets them too.
 */
export function resetPathSearchCaches(): void {
  latticeCache = null;
...
registerPlanCache(resetPathSearchCaches);
