...
grep -q 'addPostCollider(collision, drawn.trunkFoot, drawn.trunkTop, ringSizeVsRace)' $T && say "registerCollision walks the DRAWN lean (leanTrestleTree)" || bad "registerCollision closure not calling addPostCollider on the leant tree"
...
grep -q '^function leanTrestleTree' $T && grep -q 'trestleStruts(leanTrestleTree(spot.tree, drawnTree))' $T && say "the flat solve is leant only when drawn" || bad "draw loop does not lean the flat tree"
grep -q 'unplaceFromSphere(centre.clone()' test/procgen/parkFacts.ts && say "parkFacts unplaces drawn strut ends" || bad "parkFacts reads drawn struts as flat"
...
grep -q "import { Box3, InstancedMesh, Mesh, Quaternion, Vector3 } from 'three';" test/procgen/parkFacts.ts && grep -q "measureGateArch" test/procgen/parkFacts.ts && say "parkFacts: both imports" || bad "parkFacts imports"
