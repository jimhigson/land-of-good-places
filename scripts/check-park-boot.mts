...
import {
  busyLabel,
  busyMsOf as busyMsFrom,
  churn,
  controlOfCpuClock,
  cpuMs,
  describeControl,
  spin,
} from './lib/cpuClock.mts';
...
//
// Issue #606. The instrument, its three controls and the whole account of why
// it must be `process.threadCpuUsage()` rather than `process.cpuUsage()` now
// live in `scripts/lib/cpuClock.mts`, because `check:solve-cost` needed the
// same cure for the same disease and two copies of that reasoning is this
// project's most common bug. Nothing about the behaviour here changed: the
// control still runs before anything is measured, and failing it still drops
// every ceiling back to raw wall clock and says so.
// ---------------------------------------------------------------------------

const cpuClock = controlOfCpuClock();
said.push(describeControl(cpuClock));
 ...
 */
const busyMsOf = (wallMs: number, cpuDeltaMs: number): number => busyMsFrom(cpuClock, wallMs, cpuDeltaMs);
 ...
 */
const BUSY_LABEL = busyLabel(cpuClock);
