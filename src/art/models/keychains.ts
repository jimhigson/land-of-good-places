...
/**
 * Keychains — the little keyrings that dangle off the player's backpack.
 *
 * Six of them, all procedural, all tiny: a keyring about 11 cm tall, a short
 ...
 * this file: a canvas texture per keyring would cost more than the keyring.
 */

export type KeychainKind = 'ripika' | 'star' | 'strawberry' | 'rainbow' | 'heart' | 'rumi';

/**
 * How big a keyring is once it is actually **worn** on the bag, versus the
 ...
 *
 * One owner for both places a worn keyring is drawn — `entities/WornKeychain.ts`
 ...
 * a smaller keyring than the one she is about to actually wear. The shop's own
 * display rack (`world/KeychainShop.ts`'s `RACK_KEYRING_SCALE`) is a separate
 * scale of its own — a display stand, not a worn keyring, so it does not use
...
/**
 * How far a worn keyring's lowest point must clear the ground, in metres, once
 * it is scaled up by {@link KEYCHAIN_WORN_SCALE}.
 *
 * At the old, unscaled size (~20cm) every `KEYRING_HANGS` anchor
 * (`art/models/backpacks.ts`, 0.43-0.56m above the feet) had metres of slack
 * below it. At 2.5x a keyring reaches 0.5-0.6m down from its ring, which is
 * *more* than most of those anchors sit above the ground — hung literally
 * from the same point, several of the five keyrings would drag their tip
 ...
 * `check:keyring-hang`), but a per-equip vertical lift, computed from the
 * actual worn keyring's own measured height, that only ever pulls the ring
...
/**
 * How far above the keyring anchor a worn keyring's pivot must sit so its tip
 ...
 *
 * `anchorHeight` is `KEYRING_HANGS`' own Y for the bag actually worn;
 * `keyringHeight` is the un-scaled keyring's own measured `AssetHandle.height`.
 * Zero whenever the bag already has enough headroom on its own (nothing
 * moves); otherwise exactly enough to put the tip at
 * {@link KEYCHAIN_GROUND_CLEARANCE} — never more, so a keyring that fits stays
 * clipped to the bag's own corner precisely where `KEYRING_HANGS` says, and one
 * that would not fit rides no higher than it has to.
 */
export function keychainWornLift(anchorHeight: number, keyringHeight: number): number {
  const scaledDrop = keyringHeight * KEYCHAIN_WORN_SCALE;
  return Math.max(0, scaledDrop + KEYCHAIN_GROUND_CLEARANCE - anchorHeight);
}

/**
 * How a hung keyring sways when nothing is driving it dynamically — used by
 * **both** places a worn keyring dangles, again. The character-creation
 * preview's picker has no player movement to swing its keyring, so this is the
 * whole of its motion; the real worn keyring (`entities/WornKeychain.ts`)
 * layers the same two sines *under* its motion-driven pendulum springs,
 * because springs alone left a standing keyring frozen rigid at exactly 0°
...
// --- the hardware ----------------------------------------------------------
//
// Shared by every keyring so the five read as a matched set from one rack.

/** Length of the little chain between the keyring and the ring, in metres. */
...
/**
 * Adds the chain and the split ring above a keyring whose top is at `keyringTop`,
 ...
 * stray line.
 */
function addHardware(root: Group, keyringTop: number): void {
  const metal = toonMaterial(ART.cream);

  const chain = solid(
    new Mesh(new CylinderGeometry(0.006, 0.006, CHAIN_LENGTH, 6), metal),
  );
  chain.position.y = keyringTop + CHAIN_LENGTH / 2;
  root.add(chain);

  const ring = solid(
    new Mesh(new TorusGeometry(RING_RADIUS, RING_TUBE, 6, 14), metal),
  );
  ring.position.y = keyringTop + CHAIN_LENGTH + RING_RADIUS;
  root.add(ring);
}

/**
 * Drops a keyring so its lowest visible point sits on y = 0, and reports where
 * its top ended up.
 *
 * Measured rather than worked out by hand: several of these keyrings are built
 * from cones and half-tori whose extents are easy to get a centimetre wrong,
 * and a centimetre is a tenth of a keychain.
 */
function seat(keyring: Group): number {
  const bounds = visibleBounds(keyring);
  keyring.position.y -= bounds.bottom;
  return bounds.top - bounds.bottom;
}

/** Two ink dots and a pair of blush discs — the whole of a keyring's face. */
...
function ripikaKeyring(): Group {
  const keyring = new Group();
  ...
  addTinyFace(keyring, 0.004, 0.046, 0.019, 0.034, ART.ripikaCheek);
  return keyring;
}

/** A chunky five-pointed star, the same shape the plush star toy is cut from. */
function starKeyring(): Group {
  const keyring = new Group();
  // `starGeometry`'s default puts one point straight down, not the classic
  // "one point up" a star keyring needs to read right-way-up hanging off a bag
  ...
  addTinyFace(keyring, 0.004, 0.026, 0.018, 0.032);
  return keyring;
}

/** A strawberry: a little red cone with a leafy green top and seeds. */
function strawberryKeyring(): Group {
  const keyring = new Group();
  ...
  // the keyring and they are what makes a red cone read as a strawberry.
  const seedCount = 6;
  ...
  addTinyFace(keyring, 0.052, 0.045, 0.016, 0.028);
  return keyring;
}

/** Four bands of rainbow springing out of two little clouds. */
function rainbowKeyring(): Group {
  const keyring = new Group();
...
function rumiKeyring(): Group {
  ...
  addTinyFace(keyring, 0.088, 0.031, 0.013, 0.023);
  return keyring;
}

const KEYRINGS: Readonly<Record<KeychainKind, () => Group>> = {
  ripika: ripikaKeyring,
  star: starKeyring,
  strawberry: strawberryKeyring,
  rainbow: rainbowKeyring,
  heart: heartKeyring,
  rumi: rumiKeyring,
};
...
export function createKeychain(kind: KeychainKind): AssetHandle {
  ...
  const keyring = KEYRINGS[kind]();
  root.add(keyring);
  addHardware(root, seat(keyring));
