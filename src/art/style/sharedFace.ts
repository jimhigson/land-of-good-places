 ...
 *
 * {@link createFacePatch} paints a fresh set of `EXPRESSIONS` (see `faces.ts`)
 * per call, which is exactly right for a hero character and exactly wrong for
 * a shop full of identical shopkeepers: seven of them would each multiply the
 * expression count in 256² canvases on their own, and the whole game is
 * budgeted at forty (ASSET_MANIFEST.md, "texture budget").
 *
