 ...
 *
 * Three sources, merged into one screen-space direction:
 *
 * - **Touch/mouse**: a virtual stick anchored wherever the finger went down.
 *   Press and the car goes; slide your thumb and the car turns that way. Chosen
 ...
 * because only it knows where its camera is.
 */
...
/** Gamepad stick travel below which the stick is considered centred. */
    ...
    read(): SteerReading {
      ...
      for (const pad of pads) {
        ...
        if (Math.hypot(ax, ay) > PAD_DEADZONE) {
          x += ax;
          y -= ay; // pad Y is positive downwards
          wants = true;
        }
