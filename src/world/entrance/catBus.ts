 ...
 *
 * These were function-scope locals inside `createCatBus`. Nothing about them
 * varies per bus or per seed: every input is a module constant, so the values
 * were always fixed — they were merely *unreachable*, which is a different
 * thing and a far more dangerous one. See {@link CAT_BUS_DOOR_DROP}.
 */
 ...
 * space — the one owner.**
 *
 * Straight out from the step and clear of the sill. `createCatBus` returns this
 * same object as its `doorDrop` and asserts the built mesh agrees with it, so
 * there is exactly one expression for "where the door puts her down".
 *
 * **Why this is hoisted rather than left to the built bus.** A value that is
 * deterministic but *unreachable* is precisely the shape that gets hand-copied
 * into a second definition and then drifts — CLAUDE.md names that as this
 * repo's most common bug by a distance. Every input here is a module constant,
 * so this pair of numbers never varied; it was simply obtainable only from a
 * bus somebody had already built, and a caller at module scope could not get
 * it. The fix for unreachable-so-copied is to make the real value reachable,
 * never to write down a constant that "should match" it: a comment promising
 * two numbers agree is not a mechanism.
 *
 * `check:cat-bus` reads it to find the doorway a child walks through, which is
 * the first caller that needed it without a bus in hand.
 */
...
export function createCatBus(): CatBusHandle {
  ...
  // instead of letting a caller aim at a door that is not there. An assertion
  // is what makes it an owner rather than a promise.
