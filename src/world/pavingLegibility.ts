...
export interface PavingGround {
  ...
  /** True within `pad` of a railway bridge's stone — a street line there would hang off the ramp. */
  nearBridgeStone(x: number, z: number, pad: number): boolean;
...
export function offLatticeStreetRuns(
  ...
  const lineBlocked = (axis: 'x' | 'z', line: number, spanStart: number, spanEnd: number): boolean => {
    ...
    for (let s = 0; s <= steps; s += 1) {
      ...
      // so the generator keeps every such street off it. Padded by the widest
      // street's half-width and kerb (the avenue's, 1.6 + 0.425) a hair under.
      if (ground.nearBridgeStone(x, z, 1.6 + PATH_KERB_OVERHANG - 0.02)) return true;
    }
    return false;
  };
