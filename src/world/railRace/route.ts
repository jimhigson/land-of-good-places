...
import { placedEntry, type PlacedEntry } from '../parkLayout';
...
export class RailRaceRoute {
  ...
  /**
   * `chooseStart` decides where the finish arch — and so the whole ride's
   * datum — stands, given the ring and the booth's own bearing along it. In
   * the game it is the park file's recorded answer; in build tooling it is the
   * arch search (`procgen/world/railRace/plan.ts`'s `slideArchClear`).
   */
  constructor(
    stationStallId: string,
    scale: number,
    chooseStart: (route: RailRaceRoute, atBooth: number, stall: PlacedEntry) => number,
  ) {
    this.scale = scale;
    ...
    this.startDistance = chooseStart(this, atBooth, stall);
  }
